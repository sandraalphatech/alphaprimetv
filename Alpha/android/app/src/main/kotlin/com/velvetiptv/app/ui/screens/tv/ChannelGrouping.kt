...
sealed class ChannelRow {
    data class Category(val name: String, val count: Int, val locked: Boolean = false) : ChannelRow()
    data class Item(val channel: M3UChannel) : ChannelRow()
}

data class ChannelCategory(val name: String, val channels: List<M3UChannel>)
